...
def buttonClicked(txt1, txt2, txt3):
    error = ''
    df = pd.read_csv("Y:/Literator_2.0/literator22/try/Textonator/Kniga-5.csv") #загружаем файл "Y:/PyCharmProject/DjangoProject/main/Textonator/Kniga-5.csv"
    ...
    if liked_book_1.empty:
        error =("I don`t know first book name, try to check the correctness \n")
        return
    ...
    if liked_book_2.empty:
        error =("I don`t know second book name, try to check the correctness \n")
        return
    ...
    if liked_book_3.empty:
        error =("I don`t know third book name, try to check the correctness \n")
        return
    ...
    df_tag = pd.read_csv('Y:/Literator_2.0/literator22/try/Textonator/Tag_results.csv')
    persent_tag = 0
    tag_koef = 1.275
    ...
    df_d2v = pd.read_csv('Y:/Literator_2.0/literator22/try/Textonator/D2V_results.csv')
    ...
    df_semsim = pd.read_csv('Y:/Literator_2.0/literator22/try/Textonator/SemSim_results.csv')
    ...
    answer_sort = pd.DataFrame(df_answer_sort_clear, columns= ['ru_title', 'title','author', 'genre'])
    ...
    for i in range((answer_sort.shape[0])):
        ...
        for book in answer_sort_list:
            answer = {}
            if book[0] == title_1 or book[0] == title_2 or book[0] == title_2 or book[1] == title_1 or book[1] == title_2 or book[1] == title_2:
                continue
            if num > 29: break
            else: num +=1
            answer[f"b_{num}_ru_title"] = book[0]
            answer[f"b_{num}_en_title"] = book[1]
            answer[f"b_{num}_author"] = book[2]
            answer[f"b_{num}_genre"] = book[3]
            answer_books[f'{num}_book'] = answer
    print( answer_books)
